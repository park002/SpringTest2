	...
	<table class="table table-board" border="1px" width="80%" align="center">
		...
		<c:forEach items="${map.list}" var="row">
			<tr>
				<td>${row.b_no}</td>
				<td>
				<a href="#" onclick="location.href='/jaeho/board/detail?b_no=${row.b_no}'">${row.b_title}
		...
		<tr>
			<td colspan="5">
				...
				<c:if test="${map.boardPager.curBlock >1}">
	            	<a href="#" onclick="javascript:list('1')">[처음]</a>
				</c:if> 
				<!--이전 페이지 블록으로 이동 : 현재 페이지 블럭이 1보다 크면 [이전]하이퍼링크 화면에 출력  -->
				 <c:if test="${map.boardPager.curBlock >1}">
					<a href="#" onclick="javascript:list('${map.boardPager.prevPage}')">[이전]</a>
				</c:if>
				 ...
				 <c:forEach var="num" begin="${map.boardPager.blockBegin}" end="${map.boardPager.blockEnd}">
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="#" onclick="javascript:list('${num}')">${num}</a>&nbsp;
              </c:otherwise>
					</c:choose>
				</c:forEach> <!--다음 페이지 블록으로 이동 : 현재 페이지 블럭이 전체 페이지 블럭보다 작거나 같으면  [다음] 하이퍼링크를 화면에 출력  -->
				<c:if test="${map.boardPager.curBlock<=map.boardPager.totBlock }">
					<a href="#" onclick="javascript:list('${map.boardPager.nextPage}')"> [다음] </a>
				</c:if> <!--끝 페이지로 이동: 현재 페이지가 전체페이지보다 작거나 같으면 [끝] 하이퍼링크 화면에 출력  -->
				 <c:if test="${map.boardPager.curPage<=map.boardPager.totPage}">
					<a href="#" onclick="javascript:list('${map.boardPager.totPage}')">[끝]</a>
				</c:if>
			</td>
		</tr>
	</table>
