...
<c:set var="path" value="${pageContext.request.contextPath}" />
	<table style="width: 700px">
		...
		<c:forEach items="${list}" var="row">
			<tr>
				<td>회원ID:${row.replyer}	(<fmt:formatDate pattern= "yyyy-MM-dd HH:mm"  value="${row.b_date}"/>) <br> ${row.replytext} <br> 
			...
			<td>
				<!--처음 페이지로 이동: 현재 페이지가 1보다 크면 [처음]하이퍼 링크를 화면에 출력  -->
				 <c:if test="${1<replyPager.curBlock}">
					<a href="#" onclick="javascript:listReplyRest('1')">[처음]</a>
				</c:if> <!--이전 페이지로 이동:  --> 
				<c:if test="${1<replyPager.curBlock}">
					<a href="#" onclick="javascript:listReplyRest('${replyPager.prevPage}')">[이전]</a>
				...
				<c:if test="${replyPager.curBlock<=replyPager.totBlock}">
					<a href="#" onclick="javascript:listReplyRest('${replyPager.nextPage}')"> [다음]</a>
				</c:if> <!--끝 페이지로 이동: 현재 페이지가 전체페이지보다 작거나 같으면 [끝] 하이퍼링크 화면에 출력  --> 
				<c:if test="${replyPager.curPage<=replyPager.totPage}">
					<a href="#" onclick="listReplyRest('${replyPager.totPage}')">[끝]</a>
				</c:if>
			</td>
		</tr>
	</table>
